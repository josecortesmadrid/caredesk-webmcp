...
> pharmacy pickups. The agent uses **six WebMCP tools** registered straight from
> the page; the **human confirms every consequential action**; every call shows in
> an always-visible **audit trail**.
...
## The six tools (one security pattern each)
...
- *"Book the pharmacy pickup for tomorrow morning"* → `book_pharmacy_pickup` (queues, human confirms)
